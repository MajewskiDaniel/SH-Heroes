...
export const AddForm: React.FC<IAddForm> = ({id}) => {
  ...
  const [initialValuesWithId, setInitialValuesWithId] = useState<FormIEmployee | IEmployee>(INITIAL_VALUES);
  ...
  const handleSubmit = async (values: FormIEmployee | IEmployee, handlers: {
    ...
    resetForm: (nextState?: Partial<FormikState<FormIEmployee | IEmployee>>) => void,
    ...
    try {
      ...
      id ? handlers.resetForm({values: initialValuesWithId}) : handlers.resetForm({values: INITIAL_VALUES});
  ...
  >
    <Form className={styles.Form}>
      ...
      <FormItem name="level">
        ...
        <Select name="level">
          {Array.from(seniorityMap.keys()).map((key) => <Select.Option key={key} value={key}>
      ...
      <FormItem name="position">
        ...
        <Select name="position">
          {Array.from(employeePositionMap.keys()).map((key) => <Select.Option key={key} value={key}>
...
function isIEmployee (object: any): object is IEmployee {
  return Object.values(SeniorityLevel).includes(object.level)
