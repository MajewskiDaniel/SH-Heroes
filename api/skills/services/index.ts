...
export class SkillService {
  ...
  async postSkill(skill: ISkill) {
    const editedSkill = {
      ...skill,
      skillName: this.capitalizeFirstLetter(skill.skillName),
      skillCategory: this.capitalizeFirstLetter(skill.skillCategory),
    };
  ...
  async occurred(skill: ISkill) {
    const occurred = await SkillService.Skill.find({
